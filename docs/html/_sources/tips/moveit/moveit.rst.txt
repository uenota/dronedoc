...
=================================================================
このページでは、ロボット用の経路計画ライブラリである、`MoveIt! <https://moveit.ros.org/>`_ を使ってドローンの経路計画を行う手順について解説します。

インストール
-----------------------------------------------------------------
以下のコマンドでMoveIt!関連のパッケージをインストールします。
...
また、xacroファイル内の変数を置換するために、optional xacro argumentsに、 ``rotors_description_dir:=$HOME/catkin_ws/src/px4_sim_pkg/models/iris_depth_camera/xacro mavlink_udp_port:=14560 --inorder`` を指定します。
...
Virtual Joints
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Virtual Jointsを選択して仮想ジョイントの設定を行います。

.. image:: imgs/virtual_joints.png

仮想ジョイントは、ロボットのベースフレームとワールドをつなぐ仮想的なジョイントです。
Add Virtual Jointを押して仮想ジョイントを追加します。
...
Parent Frame Name
    ワールドフレームの名前
Joint Type
    ジョイントのタイプです。ドローンなのでfloatingを選択します
...
Add Jointsを選択してグループに属するジョイントを追加します。
...
Passive Joints
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
パッシブジョイントを指定するための設定です。
パッシブジョイントを指定することでプランナに対して、これらのジョイントを制御できないことを知らせることができます。
今回は使用しません。
